...
class mults {
 public:
  ...
  static const constexpr std::array<std::array<uint64_t, 9>, 64> f_lim63 =
      f_lims_it<63>();
  static const constexpr std::array<std::array<uint64_t, 9>, 57> f_lim56 =
      f_lims_it<56>();
  static const constexpr std::array<std::array<uint64_t, 9>, 49> f_lim48 =
      f_lims_it<48>();
  static const constexpr std::array<std::array<uint64_t, 9>, 41> f_lim40 =
      f_lims_it<40>();
  static const constexpr std::array<std::array<uint64_t, 9>, 33> f_lim32 =
      f_lims_it<32>();
  static const constexpr std::array<std::array<uint64_t, 9>, 25> f_lim24 =
      f_lims_it<24>();
  static const constexpr std::array<std::array<uint64_t, 9>, 17> f_lim16 =
      f_lims_it<16>();

  template <uint16_t b>
  inline static constexpr uint64_t encode(uint16_t k, uint64_t bv) {
    if constexpr (b <= 8) {
      // std::cerr << "byte f: " << std::bitset<8>(bv) << " -> " <<
      // byte_mapping<>::get_f(bv) << std::endl;
      ...
      uint16_t kp = k - ks;
      // std::cerr << b << " bit f: " << std::bitset<8>(bv >> next) << " -> " <<
      // byte_mapping<>::get_f(bv >> next) << std::endl;
  ...
  template <class T>
  inline static constexpr void bs(T& fl, uint16_t& kp, uint64_t& f_lim,
                                  uint64_t& f) {
    kp = 0;
  ...
  inline static constexpr uint64_t decode(uint16_t k, uint64_t f) {
    ...
    k -= kp;
    uint64_t md = b56[k];
    // std::cout << "63 flim: " << f_lim << ", pf: " << f / md << ", sf: " << f
    // % md << ", kp: " << kp << std::endl;
  ...
  inline static uint64_t select(uint16_t k, uint64_t f, uint16_t s) {
    return 63 -
           __builtin_clzll(_pdep_u64(uint64_t(1) << (s - 1), decode(k, f)));
  }
};
...
class h0_bv {
 ...
 public:
  h0_bv(const sdsl::bit_vector& bv) : sum_(0) {
    ...
    /*for (uint64_t i = 0; i < metap_cont.size(); ++i) {
        if (metap_cont[i] != meta_point[i]) {
            std::cerr << "s_block " << i << " offset pointer missmatch. Should
    be " << metap_cont[i] << " and is " << meta_point[i] << std::endl; exit(1);
        }
        if (metar_cont[i] != meta_rank[i]) {
            std::cerr << "s_block " << i << " partial rank missmatch. Should be
    " << metar_cont[i] << " and is " << meta_rank[i] << std::endl; exit(1);
        }
    }
    uint64_t off_cont = 0;
    for (uint64_t i = 0; i < datat_cont.size(); ++i) {
        if (i % k == 0) {
            if (meta_point[i / k] != off_cont) {
                std::cerr << "s_block " << (i / k) << " offset pointer
    missmarch. calculated: " << off_cont << ", stored: " << meta_point[i / k] <<
    std::endl; exit(1);
            }
        }
        if (datat_cont[i] != data_typ[i]) {
            std::cerr << "block " << i << " data type missmatch. Should be " <<
    datat_cont[i] << " and is " << data_typ[i] << std::endl; exit(1);
        }
        uint16_t w = widths[datat_cont[i]];
        uint64_t f_read = data_val.get_int(off_cont, w);
        if (f_read != datav_cont[i]) {
            std::cerr << "block " << i << " data value missmatch: Width = " << w
    << ", Class = " << datat_cont[i] << "\n"
                      << std::bitset<64>(f_read) << ", " << f_read << "\n"
                      << std::bitset<64>(datav_cont[i]) << ", " << datav_cont[i]
    << std::endl; exit(1);
        }
        auto enc = coder.encode(bv.get_int(i * 63, 63));
        if (datat_cont[i] != enc.first || datav_cont[i] != enc.second) {
            std::cerr << "WTF encoding pass difference... (" << enc.first << ",
    " << enc.second << ") != (" << datat_cont[i] << ", " << datav_cont[i] << ")"
    << std::endl;
        }
        off_cont += w;
    }
  ...
  uint64_t lookup_tables() const {
    uint64_t ret =
        sizeof(internal::mults::b63) + sizeof(internal::mults::b56) +
        sizeof(internal::mults::b48) + sizeof(internal::mults::b40) +
        sizeof(internal::mults::b32) + sizeof(internal::mults::b24) +
        sizeof(internal::mults::b16) + sizeof(internal::mults::b8) +
        sizeof(internal::mults::b7) + sizeof(internal::mults::f_lim63) +
        sizeof(internal::mults::f_lim56) + sizeof(internal::mults::f_lim48) +
        sizeof(internal::mults::f_lim40) + sizeof(internal::mults::f_lim32) +
        sizeof(internal::mults::f_lim24) + sizeof(internal::mults::f_lim16) +
        sizeof(widths);
    return ret;
  }
