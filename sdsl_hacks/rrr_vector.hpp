...
//          no extra dummy block added to bt_array, therefore this condition should hold
        ...
        */
        ...
        size_type C_bytes() const {
            return size_in_bytes(m_bt);
        }

        size_type F_bytes() const {
            return size_in_bytes(m_btnr);
        }

        size_type partial_sums() const {
            return size_in_bytes(m_btnrp) + size_in_bytes(m_rank) + size_in_bytes(m_invert);
        }

        size_type lookup_tables() const {
            return sizeof(rrr_helper_type::binomial::data.table) + sizeof(rrr_helper_type::binomial::data.space) + 
                   sizeof(rrr_helper_type::binomial::data.L1Mask) + sizeof(rrr_helper_type::binomial::data.O1Mask);
        }

        size_type object_bytes() const {
            return sizeof(*this);
        }
